...
class c {
  ...
  handleRequest({ request: e, event: t }) {
    ...
    let r = c && c.handler;
    const a = e.method;
    if ((!r && this.i.has(a) && (r = this.i.get(a)), !r)) return;
    let o;
    try {
      o = r.handle({
        url: s,
...
class g {
  constructor(e, t) {
    (this.h = {}),
      ...
      (this.v = [...e.plugins]),
      (this.g = new Map());
    for (const e of this.v) this.g.set(e, {});
  ...
  async cachePut(e, s) {
    const n = v(e);
    var i;
    await ((i = 0), new Promise((e) => setTimeout(e, i)));
      ...
      u = f
        ? await (async function (e, t, s, n) {
            ...
            const c = b({}, n, {
              ...
              r = await e.keys(t, c);
            for (const t of r) if (i === w(t.url, s)) return e.match(t, n);
          })(d, c.clone(), ["__WB_REVISION__"], h)
        : null;
    try {
  ...
  hasCallback(e) {
    for (const t of this.u.plugins) if (e in t) return !0;
    return !1;
  }
  ...
  *iterateCallbacks(e) {
  ...
  waitUntil(e) {
    return this.p.push(e), e;
  }
  async doneWaiting() {
    let e;
    for (; (e = this.p.shift()); ) await e;
  }
  destroy() {
    this.l.resolve();
  }
...
class m {
  ...
  async R(e, s, n) {
    ...
    try {
      if (((i = await this.N(s, e)), !i || "error" === i.type))
...
class H extends m {
  ...
  L() {
    ...
    for (const [s, n] of this.plugins.entries())
      n !== H.copyRedirectedCacheableResponsesPlugin &&
        (n === H.defaultPrecacheCacheabilityPlugin && (e = s),
        n.cacheWillUpdate && t++);
    0 === t
...
class W {
  ...
  addToCacheList(e) {
    ...
    for (const n of e) {
      ...
      if ((this.H.set(i, e), this.W.set(i, c), s.length > 0)) {
  ...
  o(
    new (class extends n {
      constructor(e, { allowlist: t = [/./], denylist: s = [] } = {}) {
        super((e) => this.P(e), e), (this.k = t), (this.M = s);
      }
      P({ url: e, request: t }) {
        if (t && "navigate" !== t.mode) return !1;
        const s = e.pathname + e.search;
        for (const e of this.M) if (e.test(s)) return !1;
        return !!this.k.some((e) => e.test(s));
      }
    })(((O = "/appShell.html"), x().createHandlerBoundToURL(O)), {
    ...
    new (class extends m {
      ...
      async N(e, s) {
        ...
        const a = await s.waitUntil(
          (async () => (await s.waitUntil(Promise.race(i))) || (await r))(),
        );
        if (!a)
          throw new t("no-response", {
            url: e.url,
      ...
      async D({ timeoutId: e, request: t, logs: s, handler: n }) {
        ...
        return (
          e && clearTimeout(e), (!i && c) || (c = await n.cacheMatch(t)), c
        );
      }
    })({
