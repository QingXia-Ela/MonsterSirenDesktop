...
function changeBackgroundStyle({
  opacity,
  blur,
}: CONFIG_TYPE['background']['backgroundOptions'][0]) {
  const bgElement =
    (document.querySelector(`#${INJECT_BACKGROUND_ID}`) as HTMLDivElement) ||
...
function getOptionByName(name?: string) {
  if (!name) return;
  return BackgroundStore.get().backgroundOptions.find(
    (item) => item.pageName === name,
  );
}
