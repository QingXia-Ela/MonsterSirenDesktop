...
const TransitionComponent = React.forwardRef<HTMLDivElement, TransitionProps<HTMLElement>>(({ in: inProp, children, ...props }, ref) => {
  return (
      ...
      {state => (
        <div
          style={{
            ...
            transform: state === 'entering' || state === 'entered' ? 'translateX(0)' : 'translateY(-.6rem)',
          }}
          ref={ref}
        >
...
const Dialog: FunctionComponent<DialogProps> = ({ className, children, title, ...props }) => {
  return (
    <MuiDialog
      classes={{
