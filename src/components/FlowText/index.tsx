import { FunctionComponent, PropsWithChildren, createRef, useEffect, useState } from "react";
...
const FlowText: FunctionComponent<FlowTextProps> = ({ children }) => {
  ...
  return (
    <div ref={outerContainer} style={{
      // @ts-expect-error: custom val
      "--move-value": `-${moveValue}px`
    }} className="w-full overflow-hidden">
      <span className={`${Styles.textContainer} whitespace-nowrap`} style={{
        animationDuration: `${moveValue / 10}s`
      }} ref={textContainer}>{children}</span>
    </div>
  );
}
