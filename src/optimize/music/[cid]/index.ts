...
function getControl() {
  return document.querySelector('div[class^="musicPlayPage__"]')?.querySelector('div[class^="control__"]') as HTMLDivElement
}
...
function getDownloadButton(wrapperClass: string, iconClass: string) {
  ...
  root.addEventListener("click", () => {
    const {
      sourceUrl,
      name
    } = SirenStore.getState().player.songDetail
    if (getExtensionByUrl(sourceUrl || "")) downloadByBrowser(sourceUrl!, `${name}${getExtensionByUrl(sourceUrl!)}`)
  })
...
let addDownloadButton = once(() => {
  ...
  const wrapperClass = prevButton.getAttribute("class")!, iconClass = (prevButton.firstChild as HTMLDivElement).getAttribute("class")!

  const downloadButton = getDownloadButton(wrapperClass, `${iconClass} iconfont icon-24gl-download`)
